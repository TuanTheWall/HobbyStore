...
--
-- Máy chủ: 127.0.0.1
-- Thời gian đã tạo: Th3 20, 2026 lúc 08:08 AM
...
('CM05', '0933456789', 'vane@example.com', 'Hải Phòng', '2024-05-02', 'Bị khóa', '2026-02-21 14:47:59', 'CM001_Legend', 'C0ffee_Lover!26');
...
('DH005', 'CM03', '2026-03-19', 'Chờ xử lý', '2026-03-19 13:08:56', 1430000, 'CM001.Alpha', '0978123456', 'Đà Nẵng', 'COD', NULL),
...
CREATE TRIGGER `trg_export_on_delivered` AFTER UPDATE ON `orders` FOR EACH ROW BEGIN
    IF NEW.status = 'Đã giao' AND OLD.status != 'Đã giao' THEN
        ...
        ON DUPLICATE KEY UPDATE
            export_num = export_num + oi.quantity;
    END IF;
END
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `trg_export_product` AFTER INSERT ON `order_item` FOR EACH ROW BEGIN
    UPDATE product_list
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_insert_history_export` AFTER INSERT ON `order_item` FOR EACH ROW INSERT INTO history (ProductID, import_num, update_date)
SELECT NEW.ProductID, NEW.quantity, o.order_date
FROM orders o
WHERE id_order = NEW.id_order
ON DUPLICATE KEY UPDATE
export_num = export_num + NEW.quantity
$$
DELIMITER ;
