...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; font-family:"Josefin Sans",sans-serif; }
    ...
    .status-update-form { display:flex; gap:8px; flex-wrap:wrap; }
    .status-btn { padding:8px 12px; border:none; border-radius:6px; cursor:pointer; font-weight:600; font-size:13px; margin-bottom:8px; }
...
<div class="wrap">
  <div class="order-header">
    ...
    <div class="order-meta">
      ...
      <div>Trạng thái: <span class="status-badge status-<?php 
        if($order['status']==='Chờ xử lý') echo 'pending';
        elseif($order['status']==='Đã xác nhận') echo 'confirmed';
        elseif($order['status']==='Đang giao') echo 'shipping';
        elseif($order['status']==='Đã giao') echo 'delivered';
        elseif($order['status']==='Đã huỷ') echo 'cancelled';
      ?>"><?php echo $order['status']; ?></span></div>
    </div>
  </div>
  ...
  <div class="info-grid">
    <div class="info-row">
      <strong>Tên khách:</strong><br>
      <span><?php echo htmlspecialchars($order['receiver_name'] ?? $order['username']); ?></span>
    </div>
    <div class="info-row">
    ...
    <div class="info-row">
      ...
      <span><?php echo htmlspecialchars($order['receiver_phone'] ?? '—'); ?></span>
    </div>
    <div class="info-row">
      <strong>Địa chỉ:</strong><br>
      <span><?php echo htmlspecialchars($order['receiver_address'] ?? '—'); ?></span>
    </div>
  </div>
  ...
  <form method="post" class="status-update-form" style="display:flex;gap:8px;flex-wrap:wrap;">
    <input type="hidden" name="id_order" value="<?php echo htmlspecialchars($order['id_order']); ?>">
    <input type="hidden" name="update_status" value="1">
    <button type="submit" name="status" value="Chờ xử lý" class="status-btn btn-pending">Chờ xử lý</button>
    <button type="submit" name="status" value="Đã xác nhận" class="status-btn btn-confirmed">Đã xác nhận</button>
    <button type="submit" name="status" value="Đang giao" class="status-btn btn-shipping">Đang giao</button>
    <button type="submit" name="status" value="Đã giao" class="status-btn btn-delivered">Đã giao</button>
    <button type="submit" name="status" value="Đã huỷ" class="status-btn btn-cancelled" onclick="return confirm('Bạn chắc chắn muốn huỷ đơn này?')">Đã huỷ</button>
  </form>
