...
if(isset($_POST['receipt_code'])){
  ...
  if(isset($_POST['product_id'])){
    ...
    for($i = 0; $i < count($product); $i++){
      ...
      // Strip any non-digit characters (removes "VNĐ", dots, etc.)
      $pr = preg_replace('/[^\d]/', '', $price[$i]);
      $q  = intval($qty[$i]);

      if($p === '' || $pr === '' || $q <= 0) continue; // skip empty rows

      $sql2 = "INSERT INTO purchase_receipt_items(receipt_code,product_id,quantity,price)
               VALUES('$receipt_code','$p','$q','$pr')";

      $conn->query($sql2);
      $q2 = "SELECT Quantity FROM product_list WHERE ProductID='$p'";
$r2 = $conn->query($q2);
$row = $r2->fetch_assoc();
$current_qty = $row['Quantity'];

$conn->query("
INSERT INTO history(ProductID,import_num,export_num,quantity,update_date)
VALUES('$p','$q',0,'$current_qty',CURDATE())
");

      $conn->query("UPDATE product_list SET Quantity = Quantity + $q WHERE ProductID='$p'");
...
<div class="wrap">
  ...
  <!-- FIX #3: single <form> wrapping everything including submit buttons -->
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function tinhTong() {
    ...
    items.forEach(item => {
      // FIX #4: strip non-digits before calculating so formatted price still works
  ...
  // Delegated events on the container
  boxContainer.addEventListener("input", function(e) {
    if (e.target.classList.contains("gia")) {
      // Strip non-digits, store raw value, then format for display
  ...
  addBtn.addEventListener("click", function() {
    ...
    // Attach input listener to new box's gia field
  ...
  // FIX #4: before submit, set all .gia values to plain numbers so PHP receives clean integers
